<template>
  <div class="fs-virtual-waterfall-container">
    ...
    <div class="flex-algin" v-if="dataState.loading && !dataState.isFinish">
      <van-loading type="spinner" size="20px" />
      <span style="margin-left: 0.4em" class="fs-14 color-grey">数据加载中～</span>
    </div>
  </div>
</template>

<script setup lang="ts">
  import { computed, markRaw, reactive, onMounted, onUnmounted, type CSSProperties } from 'vue';
  ...
  const contentRef = computed<HTMLDivElement | null>(() => props.containerRef);
  ...
  const contentStyle = computed(() => ({ height: `${computedHeight.value.maxHeight}vw` } as CSSProperties));
  ...
  const itemSizeInfo = computed(() =>
    dataState.list.reduce<Map<IDataItem['id'], IItemRect>>((pre, current) => {
      ...
      pre.set(current.id, {
        width: current.width,
        height: current.height, // Math.floor((itemWidth * current.height) / current.width),
      });
      return pre;
    }, new Map()),
  );
  ...
  const loadDataList = async () => {
    ...
    if (dataState.currentPage * requestSize.value >= total + requestSize.value) {
  ...
  const generatorItem = (item: IDataItem, before: IRenderItem | null, index: number): IRenderItem => {
    ...
    return markRaw({
      ...
      style: {
        width: `${width}vw`,
        height: `${height}vw`,
        transform: `translate3d(${index === 0 ? 0 : (width + props.gap) * index}vw, ${y}vw, 0)`,
      },
    });
  };
  ...
  const handleScroll = rafThrottle(() => {
    ...
    scrollState.start = (scrollTop / window.innerWidth) * 100;
    ...
    if (((scrollTop + clientHeight) / window.innerWidth) * 100 > computedHeight.value.minHeight) addInQueue();
  });
